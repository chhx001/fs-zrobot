...
static void setServo(UArm *uarm,char channel,short angle);
...
static void Xil_Out32(unsigned char channel,short value);
static int Xil_In32(char channel);

static void Xil_Out32(unsigned char channel,short value)
{
	struct ZRUarmReg reg_value;
	reg_value.channel = channel;
	...
	ioctl(fd,UARM_REG_WRITE,&reg_value);
	close(fd);
}
...
static void init(UArm *uarm)
{
	...
	printf("l:%d,r:%d\n",(int)uarm->attr.offsetL,(int)uarm->attr.offsetR);
